% ═══════════════════════════════════════════════════════════════
%  Research Group Presentation — 12 May 2026
...
\begin{frame}{Why Proton Therapy?}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \vspace{-4pt}
      \begin{center}
        \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{figures/Differential-depth-dose-distributions-of-photons-versus-protons-Photon-beams-have-the_W640.jpg}

        \vspace{1pt}
        {\tiny\color{gray}
         Source: Deycmar et al., ResearchGate.
         X-rays deposit maximum dose near the surface and irradiate
         all tissue along the path; protons deliver a sharp Bragg peak
         at depth and deposit near-zero dose beyond the target.}
      \end{center}
    \end{column}
    \begin{column}{0.45\textwidth}
      {\small\textbf{The physical advantage}}
      \begin{itemize}\setlength{\itemsep}{1pt}\small
        \item Protons stop abruptly at the \textbf{Bragg peak}, with
              near-zero dose beyond the tumour.
        \item Photons deposit their peak dose \emph{before} the target
              and irradiate all tissue beyond it.
        \item Result: \textbf{lower dose} to organs-at-risk (OAR),
              enabling dose escalation or reduced toxicity.
      \end{itemize}

      \vspace{3pt}
      {\small\textbf{The cost–sensitivity trade-off}}
      \begin{itemize}\setlength{\itemsep}{1pt}\small
        \item The steep Bragg peak makes dose \textbf{acutely sensitive}
              to range, anatomy, and setup uncertainties.
        \item Millimetre-level range errors can shift dose from tumour
              to OAR, which motivates \textbf{fast, accurate} dose engines.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Treatment Planning Workflow in Proton Therapy}
  \vspace{-8pt}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \centering
      \begin{tikzpicture}[
        fstep/.style={draw=tudelft, rounded corners=4pt,
                     minimum width=5.0cm, minimum height=0.55cm,
                     font=\scriptsize, text=tudelft, fill=tudelft!8,
                     align=center, inner sep=3pt},
        hilight/.style={draw=green!50!black, rounded corners=4pt,
                     minimum width=5.0cm, minimum height=0.55cm,
                     font=\scriptsize\bfseries, text=green!50!black,
                     fill=green!6, align=center, inner sep=3pt},
        adapt/.style={draw=orange!70!black, rounded corners=4pt,
                     minimum width=5.0cm, minimum height=0.55cm,
                     font=\scriptsize, text=orange!70!black, fill=orange!5,
                     align=center, inner sep=3pt},
        arr/.style={-{Stealth[length=4pt]}, thick, tudelft},
        adaptarr/.style={-{Stealth[length=4pt]}, thick, orange!70!black}
      ]
        \node[fstep] (ct)
              {\textbf{CT Scanning}};
        \node[fstep, below=5pt of ct] (delin)
              {\textbf{Structure Delineation}\\[-1pt]
               {\tiny GTV $\subset$ CTV $\subset$ PTV;\; OARs contoured by clinician}};
        \node[fstep, below=5pt of delin] (obj)
              {\textbf{Clinical Objectives}\\[-1pt]
               {\tiny patient-specific dose/volume goals for targets and OARs}};
        \node[hilight, below=5pt of obj] (dij)
              {\textbf{Spot Placement \& $d_{ij}$ Matrix Calculation}};
        \node[fstep, below=5pt of dij] (opt)
              {\textbf{Plan Optimisation}\\[-1pt]
               {\tiny find $\{w_j\}$ satisfying clinical goals:\; $D_i = \sum_j d_{ij} w_j$}};
        \node[fstep, below=5pt of opt] (qa)
              {\textbf{Quality Assurance}};
        \node[fstep, below=5pt of qa] (del)
              {\textbf{Treatment Delivery}};
        \node[adapt, below=8pt of del] (adapt)
              {\textbf{Online Adaptation} (per fraction, DAPT)\\[-1pt]
               {\tiny daily CT $\to$ re-segment $\to$ re-optimise $\to$ deliver}};
        \draw[arr]             (ct)    -- (delin);
        \draw[arr]             (delin) -- (obj);
        \draw[arr]             (obj)   -- (dij);
        \draw[arr]             (dij)   -- (opt);
        \draw[arr]             (opt)   -- (qa);
        \draw[arr]             (qa)    -- (del);
        \draw[adaptarr]        (del)   -- (adapt);
        \draw[adaptarr, dashed](adapt.east) -- ++(0.5,0) |- (dij.east);
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.45\textwidth}
      \begin{block}{The dose influence matrix $d_{ij}$}
        {\tiny
        $d_{ij}$: dose in voxel $i$ from one unit of fluence of beamlet $j$.
        The full dose field is $D_i = \sum_j d_{ij} w_j$,
        where $w_j$ are the optimised beamlet weights.

        \vspace{2pt}
        Computing $d_{ij}$ for every beamlet, beam angle, and patient anatomy
        is the \textbf{central computational bottleneck} of treatment planning.
        For a plan with thousands of beamlets, this dominates total planning time.}
      \end{block}

      \vspace{2pt}
      \begin{alertblock}{Online Adaptive Proton Therapy}
        {\tiny
        Albertini et al.\ (2024) reported the first clinical daily adaptive
        proton therapy (DAPT) workflow: an in-room CT acquired each fraction,
        plan re-optimised in under 7\,min with automated QA.
        The adapted plan was delivered in 92\,\% of fractions, improving
        OAR or target dose metrics vs.\ the non-adapted fallback.

        \vspace{2pt}
        Fast $d_{ij}$ computation is \textbf{the key enabling technology}
        for making real-time adaptation clinically feasible.}
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Current Dose Engines and Their Limitations}
  \begin{table}
    ...
    \begin{tabular}{lccl}
      ...
      \midrule
      Monte Carlo (MC) (MCsquare) & \SI{43.6}{s}/beamlet & Gold standard &
    ...
    \begin{column}{0.46\textwidth}
      \begin{center}
        ...
        \begin{tikzpicture}[
          ...
          \node[slow, below=3pt of g1]  (bev1) {Rotate per beamlet angle\\[-1pt]{\tiny(beamlet's eye view)}};
          \node[box, below=3pt of bev1]  (crop1) {Crop volume of interest};
          \node[box, below=3pt of crop1] (inf1) {Model inference};
          \node[box, below=3pt of inf1] (place1) {Place back volume};
          \node[slow, below=3pt of place1]  (bev2) {Rotate back: beamlet angle\\[-1pt]{\tiny(inverse beamlet's eye view)}};
          \node[box, below=3pt of bev2] (rot1) {Rotate $-$gantry angle\\[-1pt]{\tiny(rotate back)}};
          \draw[arr]    (g1)     -- (bev1);
          \draw[redarr] (bev1)   -- (crop1);
          \draw[arr]    (crop1)  -- (inf1);
          \draw[arr]    (inf1)   -- (place1);
          \draw[redarr] (place1) -- (bev2);
          \draw[arr]    (bev2)   -- (rot1);
          % brace marking the per-beamlet block
          \draw[brace] (bev1.north west) -- (bev2.south west)
...
\begin{frame}{The Beamlet Shape Projection $\Phi$}
  ...
  (nozzle exit) from the Beam Data Library:
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{2pt}
        ...
        \item[\textbullet] \textbf{Transformer bottleneck} captures
              ...
              which is key for modelling the Bragg peak build-up.
...
\begin{frame}{Beamlet-Level Accuracy}
  ...
  \begin{table}
    ...
    \begin{tabular}{llccccc}
      \toprule
      \textbf{Model} & \textbf{Site} & \textbf{Energy (MeV)} &
        $\boldsymbol{\Gamma}$\textbf{(1\%,3mm,0.1\%)[\%]} &
        $\boldsymbol{\Gamma}$\textbf{(1\%,3mm,10\%)[\%]} &
        $\boldsymbol{\Gamma}$\textbf{(2\%,2mm,10\%)[\%]} &
        $\boldsymbol{\Gamma}$\textbf{(1\%,2mm,3\%)[\%]} \\
      \midrule
      \multirow{2}{*}{\textbf{ADoTA} (this work)}
        & Thoracic         & {[}70, 150{]} & $99.40 \pm 0.86$ & $98.47 \pm 1.33$ & $98.33 \pm 1.83$ & $97.86 \pm 2.30$ \\
        & Abdominal/Pelvic & {[}70, 270{]} & $99.87 \pm 0.23$ & $99.46 \pm 0.63$ & $99.63 \pm 0.60$ & $99.46 \pm 0.72$ \\
      \midrule
      \multirow{3}{*}{CC-LSTM {\tiny(Neishabouri et al., 2025)}}
        & Thoracic & 48.1   & $-$ & $-$ & $-$ & $94.28 \pm 1.76$ \\
        & Thoracic & 79.2   & $-$ & $-$ & $-$ & $97.21 \pm 0.84$ \\
        & Thoracic & 102.6  & $-$ & $-$ & $-$ & $97.83 \pm 0.91$ \\
      \midrule
      \multirow{3}{*}{DoTA {\tiny(Pastor-Serrano \& Perk\'o, 2022)}}
        & Thoracic & {[}70, 140{]} & $99.46 \pm 0.81$ & $-$ & $-$ & $-$ \\
        & H\&N     & {[}70, 140{]} & $99.21 \pm 1.23$ & $-$ & $-$ & $-$ \\
        & Prostate & {[}70, 220{]} & $99.51 \pm 1.46$ & $-$ & $-$ & $-$ \\
      \midrule
      \multirow{3}{*}{MED-LSTM {\tiny(Pang et al., 2025)}}
        & Thoracic    & {[}69,\;114.5{]}   & $-$ & $99.93 \pm 0.06$ & $99.97 \pm 0.03$ & $-$ \\
        & Nasopharynx & {[}69,\;161.4{]}   & $-$ & $99.81 \pm 0.32$ & $99.92 \pm 0.19$ & $-$ \\
        & Prostate    & {[}114.5,\;200.8{]}& $-$ & $99.89 \pm 0.12$ & $99.96 \pm 0.07$ & $-$ \\
      \bottomrule
    \end{tabular}
    }
  \end{table}
  ...
  \begin{itemize}\setlength{\itemsep}{1pt}
    \item Test set: \textbf{3\,500 beamlets} (1\,400 thoracic, 2\,100 abdominal/pelvic) from 50 unseen CTs.
    \item ADoTA achieves $\geq\!99.4\%$ GPR (1\%, 3mm, 10\%) on abdominal/pelvic; thoracic is harder due to tissue heterogeneity.
  \end{itemize}
  }
\end{frame}
...
\begin{frame}{Visual Results: Lung Test Cases (Axial \& Sagittal)}
  ...
  \begin{itemize}\setlength{\itemsep}{1pt}
    ...
    \item Largest deviations are visible near \textbf{tissue
          boundaries} (rib/lung interface, mediastinum), consistent with
...
\begin{frame}{Test Set Statistics: Distribution of Metrics (3\,500 Beamlets)}
  ...
  {\scriptsize
  \begin{itemize}\setlength{\itemsep}{1pt}
    \item Violin plots and histograms of GPR (1\%, 3mm, 10\%) and RDE for all 3\,500 test beamlets.
...
\begin{frame}{Runtime: The Speed Advantage}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \vspace{6pt}
      \begin{block}{Outlook}
        Fast beamlet-shape projection can be parallelised using a GPU programming.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Why Tissue Interfaces Matter}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{table}
        ...
        \begin{tabular}{lcc}
          ...
          Heterogeneous & $98.41 \pm 1.68$ & $6.12 \pm 2.09$ \\
          \midrule
          Gap          & 0.70\,pp         & 1.01\,pp \\
          \bottomrule
        \end{tabular}
    ...
    \begin{column}{0.42\textwidth}
      ...
      \vspace{4pt}
      The model struggles where it matters most, specifically at tissue
      interfaces.
...
\begin{frame}{Why Active Learning for ADoTA?}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{4pt}
        \item The model is already working well on homogeneous regions
              (GPR $\approx$ 99.1\%).
...
\begin{frame}{Proposed Active Learning Strategy}
  ...
  \vspace{6pt}
  \begin{itemize}\setlength{\itemsep}{4pt}
    \item \textbf{Cheap to evaluate}: $\sigma_{\text{HU}}$ and TV
          require only CT data, with no model inference or MC simulation.
    \item \textbf{Physics-grounded}: directly tied to the mechanism
          (density discontinuities) that drives prediction error.
    \item \textbf{Composable}: can be combined into a single score
          and ranked across the entire unlabelled pool.
    \item \textbf{Interpretable}: each selected beamlet has a clear
          physical reason for being chosen.
  \end{itemize}
\end{frame}
