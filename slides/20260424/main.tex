...
% ── Meta ────────────────────────────────────────────────────
\title[ADoTA Heterogeneity Analysis]{Full-Database Heterogeneity Validation\\[4pt]
...
\begin{frame}{Recap \& New Approach}
  ...
  Spherical neighbourhood around BP $\rightarrow$
  six advanced HU metrics + 3-D Sobel edge metrics,
  correlated against GPR.
  Best predictor: $S_{95,\text{BP}}$
  (Spearman $\rho = {-0.391}$).

  \vspace{6pt}
  \textbf{This week: three major developments}
  \begin{enumerate}
    ...
    \item \textbf{Sphere region method}: replace the variable-length
          ...
          Two radii tested: $r = \SI{15}{mm}$ (complete) and
          $r = \SI{5}{mm}$ (in progress).
...
\begin{frame}[shrink=8]{Methodology Change: Sphere Region}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \textbf{Previous (BP-range method):}
      \begin{itemize}
        \item Proximal boundary $z_{\min}$: walk backward from BP
              to where $\text{IDD} < 0.50\,\text{IDD}_{\max}$.
        \item Distal boundary $z_{\max}$: walk forward to
              $\text{IDD} < 0.10\,\text{IDD}_{\max}$.
        \item Region spans all slices in $[z_{\min}, z_{\max}]$.
        \item \textit{Variable length}: shallow beams $\approx\SI{10}{mm}$,
              deep beams $\approx\SI{50}{mm}$.
      \end{itemize}
      ...
      \begin{itemize}
        ...
        \item Include all voxels within distance $r$ of BP:
              \begin{equation*}
                \Omega_r = \bigl\{(k,y,x) \;\big|\;
                \|\mathbf{p} - \mathbf{p}^*\|_{\mathrm{mm}} \leq r\bigr\}
              \end{equation*}
              where $\mathbf{p} = (k\Delta z, y\Delta y, x\Delta x)$.
        \item \textit{Fixed geometry}: radius $r$ is a single
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[box, below=4pt of bp] (sph)
          {Build sphere $\Omega_r$};
        \node[box, below=4pt of sph] (sob)
          {3-D Sobel on $\Omega_r$};
        \node[box, below=4pt of sob] (met)
          {Metrics: $\bar{S}$, $S_{95}$, $\Sigma S$, HU stats};
        \draw[arr] (bp) -- (sph);
        \draw[arr] (sph) -- (sob);
        \draw[arr] (sob) -- (met);
      \end{tikzpicture}
...
\begin{frame}{Dataset \& Model Inference ($n = 69\,295$)}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{center}
        ...
        \end{tabular}
      \end{center}
      {\small Pattern from last week confirmed at 18$\times$ scale:
      lower energies show larger GPR variance.}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}[shrink=5]{Overall Correlation Ranking ($n = 69\,295$, $r = \SI{15}{mm}$)}
  \begin{columns}[T]
    \begin{column}{0.44\textwidth}
      \vspace{-4pt}
    ...
    \begin{column}{0.54\textwidth}
      ...
      \vspace{4pt}
      {\small All $p \approx 0$.  \textbf{Bold}: new metric.
      Spearman $\gg$ Pearson confirms monotonic but non-linear
      relationships.\\
      \textbf{Key shift}: $\Sigma S_{\text{BP}}$ (sum Sobel) now leads
      instead of $S_{95,\text{BP}}$ — Sobel aggregation changes with
      sphere geometry.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Best Predictor: Sum Sobel in BP Sphere}
  ...
  Compare with last week: $S_{95,\text{BP}}$ led with $\rho = -0.391$
  (BP-range, $n = 3\,950$).  Results are consistent across methods and
  at $18\times$ larger scale.}
\end{frame}
...
\begin{frame}{Motivation for Energy Stratification}
  ...
  \vspace{8pt}
  \textbf{Three complementary analyses} (same as last week, now at scale):
  \begin{enumerate}
...
\begin{frame}{Spearman Correlation by Energy Bin}
  \begin{center}
    \includegraphics[height=0.74\textheight]{spearman_heatmap_by_energy.png}
  \end{center}
  \vspace{-6pt}
  ...
  at 190--220\,MeV, $\rho \approx 0$ for most metrics.}
\end{frame}
...
\begin{frame}{Partial Correlations Controlling for Energy}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      energy}. For $n_{\text{regions}}$ ($\Delta = {-0.041}$),
      controlling for energy \emph{strengthens} the correlation,
      mirroring last week's finding.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[plain]{Worst-GPR Beamlet: \texttt{1f279484},
  ...
  \begin{center}
    \includegraphics[width=\textwidth,height=0.84\textheight,keepaspectratio]%
      {1f279484-7da5-4193-9fcb-0ac81c6bf1b6_ct_seg.png}
  \end{center}
\end{frame}
}
...
\begin{frame}{Sphere Radius: \SI{15}{mm} vs \SI{5}{mm}}
  \textbf{Hypothesis}: a tighter sphere ($r = \SI{5}{mm}$) isolates
  the peak more precisely; a larger sphere ($r = \SI{15}{mm}$) captures
  more of the distal fall-off anatomy.

  \vspace{8pt}
  \begin{block}{\large Results pending}
    Run \texttt{20260421\_222950} (radius $= \SI{5}{mm}$) is currently
    processing on \texttt{cuda:2}.  Expected completion:
    \textbf{Thursday 23 April}.

    \vspace{4pt}
    This slide will be updated with:
    \begin{itemize}
      \item Side-by-side correlation table: $r = 5$ mm vs.\ $r = 15$ mm.
      \item Energy-stratified comparison.
      \item Recommendation for the default radius in future analyses.
    \end{itemize}
  \end{block}

  \vspace{6pt}
  {\small Both radii use the same model (DoTA\_v3\_grid\_search\_v11),
  same dataset ($n = 70\,145$), and identical metric definitions —
  only the spatial footprint changes.}
\end{frame}
...
\begin{frame}{Key Findings}
  \begin{enumerate}
    ...
    \vspace{4pt}
    \item \textbf{Radius sensitivity}: pending (run2 at $r = \SI{5}{mm}$).
  \end{enumerate}
\end{frame}
...
\begin{frame}{Next Steps}
  \begin{enumerate}
    \item \textbf{Sphere-radius comparison} (this week): compare
          $r = \SI{5}{mm}$ vs.\ $r = \SI{15}{mm}$ once run2 finishes.
          Determine optimal radius for downstream use.
