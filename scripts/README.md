...
| [`al_build_pool.py`](al_build_pool.py), [`al_build_validation_set.py`](al_build_validation_set.py), [`al_loop.py`](al_loop.py) | `config_al.yaml`, `config_al_train.yaml` | **Active-learning loop**: split held-out CTs into pool and validation roles → generate the frozen difficulty-balanced validation set → run one arm per sampling strategy (score, label with Monte Carlo, retrain, validate) against a Monte Carlo budget. Smoke test: the same config plus `--set KEY=VALUE` overrides, listed in the guide. | [docs/al_loop.md](docs/al_loop.md) |
| [`al_retro_loop.py`](al_retro_loop.py), [`al_compare.py`](al_compare.py) | `config_al_retro_loop.yaml`, `config_al_compare.yaml` | **Retrospective active-learning benchmark** on the training HDF5: apply the exclusion list and freeze the splits → train the shared cycle-0 baseline → run one sampling strategy per process (score the pool input-only, select, grow, train, validate) → compare the runs. Smoke test: the same config plus `--set KEY=VALUE` overrides, listed in the guide. | [docs/al_retro_loop.md](docs/al_retro_loop.md) |
| [`build_beamlet_h5.py`](build_beamlet_h5.py) | none | **Rebuild the training HDF5**: raw Monte Carlo records → v3 schema with full provenance. Applies the exclusion list, runs the pinned recipe (flux projection, normalisation, downsampling), writes three datasets and metadata attributes per record, plus side-output index and skip CSVs. Resume on interrupt with `--resume`. | [docs/build_beamlet_h5.md](docs/build_beamlet_h5.md) |
| [`check_beamlet_h5.py`](check_beamlet_h5.py) | none | **Verify a rebuilt HDF5** against its source: id-set arithmetic, dataset equality and storage parameters, attribute types and values, v3-only attributes present, spot_key distribution, H5PYGenerator equivalence on sampled records. Supports partial (smoke) builds with `--ids-file`. | [docs/build_beamlet_h5.md](docs/build_beamlet_h5.md) |
