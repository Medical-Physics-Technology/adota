...
| [`gamma_beamlet_benchmark.py`](gamma_beamlet_benchmark.py) | CLI flags | The same four-rung ladder at beamlet scale (160 x 30 x 30): parity against `pymedphys.gamma`, time per beamlet, and the cost of one gamma pass over a validation pool. | [docs/gamma_beamlet_benchmark.md](docs/gamma_beamlet_benchmark.md) |
| [`gamma_pool_benchmark.py`](gamma_pool_benchmark.py), [`gamma_scaling_benchmark.py`](gamma_scaling_benchmark.py), [`gamma_profile_beamlet.py`](gamma_profile_beamlet.py), [`gamma_plan_map_agreement.py`](gamma_plan_map_agreement.py) | CLI flags | The EXP-0008 evidence: directly timed beamlet pools, controlled size scaling on nested plan crops, profiler attribution of a beamlet evaluation, and plan-scale map agreement in native precision. | [docs/gamma_evidence.md](docs/gamma_evidence.md) |
| [`analysis/report_gamma_acceleration.py`](analysis/report_gamma_acceleration.py), [`analysis/audit_gamma_report.py`](analysis/audit_gamma_report.py) | CLI flags | Assemble the gamma benchmarks into the technical report's tables, figures, CSVs and headline-number macros; audit the macros against the CSVs. | [docs/gamma_evidence.md](docs/gamma_evidence.md) |
