...
The **end-to-end plan-level dose pipeline**: a plan directory → per-spot ADoTA
inputs → batched inference → accumulated full-grid dose → validation against the
Monte-Carlo reference (dose comparison, DVH, gamma). It is the bridge from the
per-beamlet model to a clinically meaningful **plan** dose, and runs either
**staged** (`extract,infer,accumulate`) or **fused/disk-free** (`stream`, ~2×
faster, numerically identical dose), optionally on a **2 mm field grid**
(`grid_factor: 2`, ~2.6× faster with the dose preserved).
...
📖 **Full guide** — what plan data you need and where to put it (DICOM reader
planned), the two execution modes, the `grid_factor` 2 mm mode, the complete
config + CLI reference, outputs, and the batch helpers:
**[scripts/docs/run_plan_opentps.md](docs/run_plan_opentps.md)**.
