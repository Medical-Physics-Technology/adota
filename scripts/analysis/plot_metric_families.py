...
S = dict(min_ct=-1024., max_ct=3071., min_ds=0., max_ds=25277028.); RES = (2., 2., 2.); DZ = 2.0
...
i = list(ds.record_ids).index(UUID); x, _e, y = ds[i]
ct = inverse_minmax(x[0].numpy(), S["min_ct"], S["max_ct"]); flux = x[1].numpy()
dose = inverse_minmax(np.squeeze(y.numpy()), S["min_ds"], S["max_ds"])
D, H, W = ct.shape; midy = H // 2
z_min, z_max = estimate_bp_range(ct, dose); k0, k1 = int(np.ceil(z_min)), int(np.floor(z_max))
...
def profile(a, b):
    ...
    for j, k in enumerate(range(a, b)):
        ...
        if fm < 1e-12: continue
        m = fs >= 0.10 * fm
        if m.sum(): prof[j] = np.average(ct[k][m], weights=fs[m])
    return prof
Hk = profile(0, D)
...
axI.set_ylabel("lateral [vox]"); axI.set_title(
    "INPUT: CT (grayscale) + beam flux (orange) — sagittal.  The metrics read the density ALONG the beam.",
    fontsize=11.5, color=INK); axI.tick_params(labelbottom=False)
...
# sigma band
mu = np.nanmean(Hk[k0:k1]); sg = adv["sigma_hu_bp"]
...
jm = int(np.argmax(diffs)) + 1; xj = regions[jm]["start_slice"] * DZ
axP.annotate("", xy=(xj, regions[jm]["mean_hu"]), xytext=(xj, regions[jm-1]["mean_hu"]),
             arrowprops=dict(arrowstyle="<->", color=RED, lw=2.2))
axP.annotate(f"max_hu_jump\n= {adv['max_hu_jump']:.0f} HU", xy=(xj, np.mean([regions[jm]['mean_hu'], regions[jm-1]['mean_hu']])),
...
axP.set_xlabel("depth along beam [mm]"); axP.set_ylabel("H(k): flux-weighted mean HU")
...
axI.set_xlim(0, D*DZ + 10); axP.set_xlim(0, D*DZ + 10)
# tissue legend and metric summary placed BELOW the x-axis, so neither overlaps the profile
seen = {}; [seen.setdefault(int(segment_hu(np.array([h]))[0]), True) for h in Hk if np.isfinite(h)]
...
fig.savefig(OUT / "family_hu_profile.png", dpi=150); plt.close(fig); print("family_hu_profile.png")


# ============================ FAMILY B: WEPL ============================
rsp = hu_to_rsp_mcsquare(ct)
wepl = compute_wepl_map(ct, RES, bp_idx * DZ); pf = compute_pflugfelder_hi(wepl, flux.sum(axis=0))
mask2d = flux.sum(axis=0) > 0.10 * flux.sum(axis=0).max()
fig = plt.figure(figsize=(13, 4.6)); gs = GridSpec(1, 4, width_ratios=[1, 1, 0.9, 0.7],
    wspace=0.42, left=0.05, right=0.97, top=0.82, bottom=0.16)
a0 = fig.add_subplot(gs[0]); im0 = a0.imshow(sag(ct), cmap="gray", origin="lower", aspect="auto", vmin=-1000, vmax=800)
a0.set_title("1) CT (HU)"); a0.set_xlabel("depth [vox]"); a0.set_ylabel("lateral")
a1 = fig.add_subplot(gs[1]); im1 = a1.imshow(sag(rsp), cmap="magma", origin="lower", aspect="auto", vmin=0, vmax=1.8)
a1.set_title("2) RSP = ρ·S_mat/S_water"); a1.set_xlabel("depth [vox]")
fig.colorbar(im1, ax=a1, fraction=0.046, pad=0.04)
a2 = fig.add_subplot(gs[2]); wp = np.where(mask2d, wepl, np.nan)
im2 = a2.imshow(wp, cmap="viridis", origin="lower", aspect="auto"); a2.set_title("3) WEPL map  W(i,j)")
a2.set_xlabel("lat x"); a2.set_ylabel("lat y"); fig.colorbar(im2, ax=a2, fraction=0.046, pad=0.04, label="mm")
a3 = fig.add_subplot(gs[3]); vals = wepl[mask2d]
...
a3.set_title(f"WEPL spread\nstd={pf['wepl_std']:.1f} mm"); a3.set_xlabel("count")
...
fig.savefig(OUT / "family_wepl.png", dpi=150); plt.close(fig); print("family_wepl.png")


# ============================ FAMILY C: edges (Sobel + structure tensor) ============================
gmag = np.sqrt(ndsobel(ct, 0)**2 + ndsobel(ct, 1)**2 + ndsobel(ct, 2)**2)
fig = plt.figure(figsize=(13, 4.6)); gs = GridSpec(1, 3, width_ratios=[1, 1, 1],
    wspace=0.3, left=0.05, right=0.965, top=0.82, bottom=0.16)
b0 = fig.add_subplot(gs[0]); b0.imshow(sag(ct), cmap="gray", origin="lower", aspect="auto", vmin=-1000, vmax=800)
b0.set_title("1) CT (HU)"); b0.set_xlabel("depth [vox]"); b0.set_ylabel("lateral")
b1 = fig.add_subplot(gs[1]); im = b1.imshow(sag(gmag), cmap="hot", origin="lower", aspect="auto")
b1.set_title("2) |∇HU| (3-D Sobel)"); b1.set_xlabel("depth [vox]"); fig.colorbar(im, ax=b1, fraction=0.046, pad=0.04)
# along-beam vs across-beam gradient share
gz = np.abs(ndsobel(ct, 0)); glat = np.sqrt(ndsobel(ct, 1)**2 + ndsobel(ct, 2)**2)
b2 = fig.add_subplot(gs[2])
seg = slice(k0, k1 + 1); m = (flux[seg].sum() > 0)
along = float(gz[seg].sum()); across = float(glat[seg].sum())
b2.bar(["across beam\n(lateral)", "along beam\n(axial)"], [across, along], color=[RED, BLUE], width=0.6)
b2.set_title("3) edge orientation\n(structure tensor → anisotropy, θ)"); b2.set_ylabel("Σ|∇HU| in BP zone")
b2.text(0, across, f"lateral_edge_energy\n∝ tr(J)·sin²θ", ha="center", va="bottom", fontsize=8.5, color=RED)
fig.suptitle("Edge family — |∇HU| highlights the tissue boundaries the beam crosses",
             fontsize=13, fontweight="bold", y=0.995)
fig.savefig(OUT / "family_edges.png", dpi=150); plt.close(fig); print("family_edges.png")
print("done")
