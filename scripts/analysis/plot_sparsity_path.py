"""Figure: how many metrics the difficulty score actually needs.

Plots the patient-grouped held-out Spearman correlation of the acquisition
score against the number of metrics retained along the Lasso sparsity path,
marking the chosen 14-metric interpretable model. Values are the results
recorded in ``research/acquisition_function_final_summary.md``.
"""
...
nf=[1,2,8,9,14,30]
sp=[0.718,0.732,0.775,0.786,0.799,0.856]
fig,ax=plt.subplots(figsize=(7.2,4.6))
ax.axhline(0.947,ls=":",lw=1.6,color=MUTED)
ax.text(30,0.951,"GBM (non-linear) ceiling  0.95",ha="right",va="bottom",color=MUTED,fontsize=9.5)
ax.axhline(0.80,ls="--",lw=1.5,color=RED)
ax.text(1,0.806,"target 0.80",color=RED,fontsize=9.5,va="bottom")
...
ax.set_xlabel("number of metrics kept in the score")
ax.set_ylabel("held-out Spearman (patient-grouped)")
ax.set_title("Sparsity path — how many metrics the score needs",fontsize=12.5,pad=8)
ax.set_ylim(0.68,0.98)
ax.set_xlim(0,31)
ax.grid(color=GRID,lw=0.7)
fig.tight_layout()
fig.savefig("/home/mstryja/projects/adota/research/figures/acquisition/fig_sparsity_path.png",dpi=150)
