...
def run_inference(
    ...
    for batch in batches:
        # Each record is the 2-channel (CT, flux) input + normalized energy.
        load_t = perf_counter()
        records = [
        ...
        for i, spot_id in enumerate(batch):
            save_prediction(pred[i].unsqueeze(0), spot_id, storage, scale=config.scale)
        save_s += perf_counter() - save_t
    ...
    logger.info(
        "Inference complete: %d spots | record-load %.1fs, forward %.1fs "
        "(%.2f ms/spot), save %.1fs | total %.1fs",
        n,
        load_s,
        forward_s,
