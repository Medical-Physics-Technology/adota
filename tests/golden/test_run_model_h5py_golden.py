"""Characterization (golden) test for ``scripts/run_model_h5py.py``.
...
This baseline is captured against the CURRENT code, which contains the known
MAPE argument-order bug. The agreed MAPE fix (modification #3) will change the
``mape_pct`` column; the golden is re-captured at that point and the diff is
reviewed explicitly. Every other column must stay put.

The harness calls ``evaluate_single_sample`` directly on a small
``H5PYGenerator`` built from a fixed slice of record ids.
...
def test_run_model_h5py_golden(tmp_path):
    gl.set_determinism()

    from scripts.run_model_h5py import evaluate_samples, save_results_csv
    ...
    # Drive the refactored engine path.
    ...
    status, msgs = gl.check_against_golden("run_model_h5py", out.read_text())
    if status == "captured":
