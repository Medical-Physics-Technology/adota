...
def test_dose_in_body_fraction():
    mask = np.zeros((10, 10, 10), dtype=np.float32); mask[:, :, 5:] = 1.0   # right half in body
    dose = np.zeros((10, 10, 10), dtype=np.float32); dose[:, :, 6] = 1.0    # all dose in body
    assert dose_in_body_fraction(dose, mask) == 1.0
    dose2 = np.zeros_like(dose); dose2[:, :, 1] = 1.0                       # all dose in air
    assert dose_in_body_fraction(dose2, mask) == 0.0
    dose3 = np.zeros_like(dose); dose3[:, :, 1] = 1.0; dose3[:, :, 6] = 3.0  # 3/4 in body
